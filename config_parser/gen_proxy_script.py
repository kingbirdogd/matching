...
"""
  ...
  for i in j['instruments']:
    ...
    if i["book_name"] == "Futures":
      ...
      outStr += f"taskset -c {cpui} ${{CORE_LOCATION}}/test_md_tcp_server {i['factor']} 127.0.0.1 34671 35671 127.0.0.1 {tsz} {fees} 127.0.0.1 34673 127.0.0.1 34672 a_bid_b_bid   a_ask_b_ask   add_bid_implier      add_ask_implier   >> ${{LOG_LOCATION}}/md_tcp1.out.log 2>> ${{LOG_LOCATION}}/md_tcp1.err.log &\nsleep 1\n"
      cpui += 1; cpui %= 16
      ...
      fees = i['impliers'][0]['ai_maker_fees'] if len(i['impliers']) > 0 else 0.0  # assume only 1 implier
      outStr += f"taskset -c {cpui} ${{CORE_LOCATION}}/test_md_tcp_server {i['factor']} 127.0.0.1 34672 35672 127.0.0.1 {tsz} {fees} 127.0.0.1 34671 127.0.0.1 34673 a_bid_b_ask   a_ask_b_bid   minus_bid_implier    minus_ask_implier >> ${{LOG_LOCATION}}/md_tcp2.out.log 2>> ${{LOG_LOCATION}}/md_tcp2.err.log &\nsleep 1\n"
      cpui += 1; cpui %= 16
      ...
      fees = i['impliers'][0]['ai_maker_fees'] if len(i['impliers']) > 0 else 0.0  # assume only 1 implier
      outStr += f"taskset -c {cpui} ${{CORE_LOCATION}}/test_md_tcp_server {i['factor']} 127.0.0.1 34673 35673 127.0.0.1 {tsz} {fees} 127.0.0.1 34671 127.0.0.1 34672 a_bid_b_ask   a_ask_b_bid   minus_bid_implier    minus_ask_implier >> ${{LOG_LOCATION}}/md_tcp3.out.log 2>> ${{LOG_LOCATION}}/md_tcp3.err.log &\nsleep 1\n"
      cpui += 1; cpui %= 16
      ...
      fees = i['impliers'][0]['ai_maker_fees'] if len(i['impliers']) > 0 else 0.0  # assume only 1 implier
      outStr += f"taskset -c {cpui} ${{CORE_LOCATION}}/test_md_tcp_server {i['factor']} 127.0.0.1 34674 35674 127.0.0.1 {tsz} {fees} 127.0.0.1 34672 127.0.0.1 34675 a_bid_b_bid   a_ask_b_ask   repo_out_bid_implier repo_out_ask_implier  >> ${{LOG_LOCATION}}/md_tcp4.out.log 2>> ${{LOG_LOCATION}}/md_tcp4.err.log &\nsleep 1\n"
      cpui += 1; cpui %= 16
      ...
      fees = i['impliers'][0]['ai_maker_fees'] if len(i['impliers']) > 0 else 0.0  # assume only 1 implier
      outStr += f'taskset -c {cpui} ${{CORE_LOCATION}}/test_md_tcp_server {i["factor"]} 127.0.0.1 34675 35675 127.0.0.1 {tsz} {fees}       ""     0        ""     0 a_none_b_none a_none_b_none none                 none               >> ${{LOG_LOCATION}}/md_tcp5.out.log 2>> ${{LOG_LOCATION}}/md_tcp5.err.log &\nsleep 1\n'
      cpui += 1; cpui %= 16
      ...
      fees = i['impliers'][0]['ai_maker_fees'] if len(i['impliers']) > 0 else 0.0  # assume only 1 implier
      outStr += f'taskset -c {cpui} ${{CORE_LOCATION}}/test_md_tcp_server {i["factor"]} 127.0.0.1 34676 35676 127.0.0.1 {tsz} {fees}       ""     0        ""     0 a_none_b_none a_none_b_none none                 none               >> ${{LOG_LOCATION}}/md_tcp6.out.log 2>> ${{LOG_LOCATION}}/md_tcp6.err.log &\nsleep 1\n'
      cpui += 1; cpui %= 16

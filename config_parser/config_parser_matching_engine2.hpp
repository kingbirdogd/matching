...
const char* book_key = "market_code";
...
public:
  ...
  static auto my_read_inst_config(const nl::json &j, std::unordered_map<std::string, matching::engine*> &book_map) {
    ...
    std::clog << "Setting up book: " << j[book_key] << '\n';
    ...
    const auto [it, success] = book_map.insert({j[book_key], &book});
    if (!success) {
      std::clog << j[book_key] << " is defined more than once!!!" << '\n';
      exit(-1);
    }
    ...
    auto res = [s, &j](std::unordered_map<std::string, matching::engine*> &book_map){
      ...
      std::clog << "Setting up impliers for book: " << j[book_key] << '\n';
      if (j["impliers"].size() == 0)
        engines.push_back(s);

      for (auto& implier: j["impliers"]) {
        std::clog << "  implier: " << implier << '\n';
        if (!assert_books_exist(j[book_key], implier, book_map)) {
          engines.push_back(s);
          continue;
        }
        ...
        if (implier["bi_type"].get<std::string>().compare("a_bid_implier") == 0 &&
          ...
          std::clog << "bid_implier and ask_implier are not both defined properly for " << j[book_key] << '\n';
        }
      }
      return engines;
    };
    std::clog << "Done " << j[book_key] << '\n';
    return res;

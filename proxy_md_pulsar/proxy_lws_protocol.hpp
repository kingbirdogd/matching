...
//      lwsl_notice("hd %s\n", hd);
      ...
      pss->client_ptr = std::make_shared<proxy::Client>(std::move(dummy_socket), wsi, vhd, ip_address.c_str(), v1, num_queue, ip_address, _client_connections_map, _client_connections_map_mutex);
    }
      break;

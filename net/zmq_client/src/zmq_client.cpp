...
void zmq_client::_check_connect()
{
	struct pollfd fd;
	fd.fd = _sock;
	fd.events = POLLOUT;
...
zmq_client::zmq_client(const std::string& host, unsigned short int port):
		_buffer(),
		_msg_cb(),
		_on_connected(),
		_on_disconnected(),
...
zmq_client::zmq_client(int sock,const std::string& host, unsigned short int port):
		_buffer(),
		_msg_cb(),
		_on_connected(),
		_on_disconnected(),
...
zmq_client::zmq_client(zmq_client&& client):
...
zmq_client& zmq_client::operator=(zmq_client&& client)
{
	_buffer = std::move(client._buffer);
	_msg_cb = std::move(client._msg_cb);
	_on_connected = std::move(client._on_connected);
...
void zmq_client::send(const void* ptr, std::size_t size)
{
...
void zmq_client::set_on_msg(msg_cb&& msg_cb)
{
	_msg_cb = std::move(msg_cb);
}

void zmq_client::set_connected(event_cb&& on_connected)
{
	_on_connected = std::move(on_connected);
}

void zmq_client::set_disconnected(event_cb&& on_disconnected)
{
	_on_disconnected = std::move(on_disconnected);
}

const std::string& zmq_client::get_host()
{
	return _host;
}

unsigned short int zmq_client::get_port()
{
	return _port;
}

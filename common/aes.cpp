...
static const uint32_t encrypt_table[256] = {
...
static const uint8_t decrypt_sbox[256] = {
...
static const uint32_t decrypt_table[256] = {
...
template <size_t Key_Size>
constexpr size_t AESBase<Key_Size>::output_block_size;
template <size_t Key_Size>
constexpr size_t AESBase<Key_Size>::key_size;
template <size_t Key_Size>
constexpr size_t AESBase<Key_Size>::sched_size;

template <size_t Key_Size>
size_t AESEncrypterBase<Key_Size>::process(uint8_t (&out)[16], const uint8_t in[], size_t n __unused) noexcept {
	assert(n == 16);
...
AESDecrypterBase<Key_Size>::AESDecrypterBase(const AESEncrypterBase<Key_Size> &encrypter) noexcept {
	...
	for (size_t i = 0; i < 4; ++i) {
		this->key_schedule[AESBase<Key_Size>::sched_size - 4 + i] = encrypter.key_schedule[i];
	}
}

template <size_t Key_Size>
size_t AESDecrypterBase<Key_Size>::process(uint8_t (&out)[16], const uint8_t in[], size_t n __unused) noexcept {
	assert(n == 16);
	uint32_t s0 = reinterpret_cast<const be<uint32_t> *>(in)[0] ^ this->key_schedule[0];
	uint32_t s1 = reinterpret_cast<const be<uint32_t> *>(in)[1] ^ this->key_schedule[1];
	uint32_t s2 = reinterpret_cast<const be<uint32_t> *>(in)[2] ^ this->key_schedule[2];
...
AES128Encrypter::AES128Encrypter(const uint8_t (&key)[16]) noexcept {
	...
	for (size_t i = 0; i < 10; ++i) {
		key_schedule[i * 4 + 4] =
				key_schedule[i * 4] ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 4 + 3] >> 16)] << 24 ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 4 + 3] >>  8)] << 16 ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 4 + 3] >>  0)] <<  8 ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 4 + 3] >> 24)] <<  0 ^
				round_constants[i] << 24;
		key_schedule[i * 4 + 5] = key_schedule[i * 4 + 1] ^ key_schedule[i * 4 + 4];
		key_schedule[i * 4 + 6] = key_schedule[i * 4 + 2] ^ key_schedule[i * 4 + 5];
		key_schedule[i * 4 + 7] = key_schedule[i * 4 + 3] ^ key_schedule[i * 4 + 6];
	}
}
...
AES192Encrypter::AES192Encrypter(const uint8_t (&key)[24]) noexcept {
	...
	for (size_t i = 0;;) {
		key_schedule[i * 6 + 6] =
				key_schedule[i * 6] ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 6 + 5] >> 16)] << 24 ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 6 + 5] >>  8)] << 16 ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 6 + 5] >>  0)] <<  8 ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 6 + 5] >> 24)] <<  0 ^
				round_constants[i] << 24;
		key_schedule[i * 6 + 7] = key_schedule[i * 6 + 1] ^ key_schedule[i * 6 + 6];
		key_schedule[i * 6 + 8] = key_schedule[i * 6 + 2] ^ key_schedule[i * 6 + 7];
		key_schedule[i * 6 + 9] = key_schedule[i * 6 + 3] ^ key_schedule[i * 6 + 8];
		if (++i == 8) {
			break;
		}
		key_schedule[i * 6 + 4] = key_schedule[i * 6 - 2] ^ key_schedule[i * 6 + 3];
		key_schedule[i * 6 + 5] = key_schedule[i * 6 - 1] ^ key_schedule[i * 6 + 4];
	}
}
...
AES256Encrypter::AES256Encrypter(const uint8_t (&key)[32]) noexcept {
	...
	for (size_t i = 0;;) {
		key_schedule[i * 8 + 8] =
				key_schedule[i * 8] ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 8 + 7] >> 16)] << 24 ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 8 + 7] >>  8)] << 16 ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 8 + 7] >>  0)] <<  8 ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 8 + 7] >> 24)] <<  0 ^
				round_constants[i] << 24;
		key_schedule[i * 8 +  9] = key_schedule[i * 8 + 1] ^ key_schedule[i * 8 +  8];
		key_schedule[i * 8 + 10] = key_schedule[i * 8 + 2] ^ key_schedule[i * 8 +  9];
		key_schedule[i * 8 + 11] = key_schedule[i * 8 + 3] ^ key_schedule[i * 8 + 10];
		if (++i == 7) {
			break;
		}
		key_schedule[i * 8 + 4] =
				key_schedule[i * 8 - 4] ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 8 + 3] >> 24)] << 24 ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 8 + 3] >> 16)] << 16 ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 8 + 3] >>  8)] <<  8 ^
				encrypt_sbox[static_cast<uint8_t>(key_schedule[i * 8 + 3] >>  0)] <<  0;
		key_schedule[i * 8 + 5] = key_schedule[i * 8 - 3] ^ key_schedule[i * 8 + 4];
		key_schedule[i * 8 + 6] = key_schedule[i * 8 - 2] ^ key_schedule[i * 8 + 5];
		key_schedule[i * 8 + 7] = key_schedule[i * 8 - 1] ^ key_schedule[i * 8 + 6];
	}
}


template class AESEncrypterBase<16>;
template class AESDecrypterBase<16>;
template class AESEncrypterBase<24>;
template class AESDecrypterBase<24>;
template class AESEncrypterBase<32>;
template class AESDecrypterBase<32>;


#include "cipher.tcc"

template class CTR<AES128Encrypter>;
template class CTR<AES192Encrypter>;
template class CTR<AES256Encrypter>;


#include "codec.tcc"

template class CodecSource<CTR<AES128Encrypter>>;
template class CodecSink<CTR<AES128Encrypter>>;
template class CodecSource<CTR<AES192Encrypter>>;
template class CodecSink<CTR<AES192Encrypter>>;
template class CodecSource<CTR<AES256Encrypter>>;
template class CodecSink<CTR<AES256Encrypter>>;
